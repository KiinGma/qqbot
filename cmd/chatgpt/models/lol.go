...
func (bd *BattleDetail) GetPlayerDetailById(openid string) *PlayerDetail {
	for _, detail := range bd.PlayerDetails {
		if detail.Openid == openid {
			return &detail
		}
	}
	return nil
...
func (bd *BattleDetail) GetTeamDetailById(teamId string) *TeamDetail {
	for _, detail := range bd.TeamDetails {
